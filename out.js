(() => {
  ...
  // src/point.js
  var Point = class {
    constructor(hits, collisions, didIWin2, isServer) {
      this.hits = hits;
      this.collisions = collisions;
      this.isServer = isServer;
      this.didIWin = didIWin2;
    }
    get isServiceFault() {
  ...
  var Collision = class {
    constructor(with_, vx, vy, vz, rx, ry, rz, posx, posy, posz, lastHit, isServe, offNet) {
      this.with = with_;
      this.vx = vx;
      this.vy = vy;
      this.vz = vz;
  ...
  var Hit = class {
    constructor(vx, vy, vz, rx, ry, rz, posx, posy, posz) {
      ...
      this.posy = posy;
      this.posz = posz;
      this.metersPerSecond = magnitude2(vx, vy, vz);
      this.revolutions = magnitude2(rx, ry, rz);
    }
    get isForehand() {
      return this.posx > 0;
    }
  ...
  var PlaySession = class {
    ...
    allHits() {
      ...
      this.matches.forEach(
        (m) => m.rounds.forEach(
          (r) => r.points.forEach((p) => p.hits.forEach((h) => hits.push(h)))
        )
      );
      return hits;
    }
  ...
  var play_session_default = PlaySession;

  // src/play_sessions.js
  var import_sum = __toESM(require_sum(), 1);
  var import_groupBy = __toESM(require_groupBy(), 1);
  var PlaySessions = class _PlaySessions {
    constructor(sessions2) {
      this.sessions = sessions2;
    }
    byWeek() {
      const grouped = (0, import_groupBy.default)(this.sessions, "weekStartDateString");
      const dates = Object.keys(grouped);
      dates.map((date) => {
        grouped[date] = new _PlaySessions(grouped[date]);
      });
      return grouped;
    }
    lastWeek() {
      ...
      const lastWeekDate = new Date(
        currentDate.getTime() - 7 * 24 * 60 * 60 * 1e3
      );
      return this.sessions.filter((s) => s.date > lastWeekDate);
    }
    get matchesWon() {
      return (0, import_sum.default)(this.sessions.map((s) => s.matchesWon));
    }
    ...
    allMyHitsToTable(forehand = true, backhand = true, net = true, serve = true, rally = true) {
      return this.allPoints.map((p) => {
        return p.collisions.map((c, i) => {
          if (c.with === "TheirTable") {
            ...
            if (!serve)
              exclude = exclude || c.lastHit?.isServe;
            if (!rally)
              exclude = exclude || !c.lastHit?.isServe;
            if (!net)
              exclude = exclude || c.offNet;
            if (!exclude) {
              return c;
            }
          }
    ...
    get allPoints() {
      ...
      this.sessions.forEach((s) => {
        const matches = filterOpponent ? s.matches.filter((m) => m.opponent === filterOpponent) : s.matches;
        matches.forEach((m) => {
    ...
    get consistency() {
      ...
      this.allPoints.forEach((p) => {
        p.collisions.forEach((c) => {
          if (c.with === "MyHit")
            hit++;
          if (c.with === "MyHit" && c.unforcedError)
            errors++;
        });
      });
      return 1 - errors / hit;
    ...
    get lastTableHitBeforeUnforcedError() {
      return this.allLostPoints.map((p) => {
        let lastHit;
        let lastTableHit;
        p.collisions.forEach((c) => {
  ...
  var play_sessions_default = PlaySessions;

  // src/parser.js
  var xyzParser = (anchor) => {
    const number = "(-?\\d+(?:.\\d+(?:E-\\d+)?)?)";
    const regexString = `\\(${number},${number},${number}\\)`;
    const re = new RegExp(`${anchor}${regexString}`);
    return re;
  };
  var getCollidedWith = (collision, isFirst) => {
  ...
  var cleanUpCollisions = (collisions) => {
    while (true) {
  ...
  var addUnforcedError = (collisions) => {
    let lastHit;
    let hitIndex = 0;
    let tableIndex = 0;
    ...
    if (hitIndex > tableIndex)
      return lastHit.unforcedError = true;
  };
  var collisionParser = (point, isFirst) => {
    ...
    const collisions = collisionChunks.map((collision) => {
      ...
      const isServe = (collidedWith === "MyHit" || collidedWith === "TheirHit") && !firstHitHappened;
      if (isServe) {
        firstHitHappened = true;
      }
      const offNet = lastCollision?.with === "Net";
      const collisionObj = new collision_default(
        collidedWith,
        parseFloat(vel?.[1] || 0),
        parseFloat(vel?.[2] || 0),
        parseFloat(vel?.[3] || 0),
        parseFloat(rrate?.[1]) / 360,
        parseFloat(rrate?.[2]) / 360,
        parseFloat(rrate?.[3]) / 360,
        parseFloat(pos?.[1] || 0),
        parseFloat(pos?.[2] || 0),
        parseFloat(pos?.[3] || 0),
        lastHit,
  ...
  var didIServe = (serverId, playerId) => {
    return parseInt(serverId) === playerId;
  };
  var didIWin = (roundScore, lastRoundScore, isFirst) => {
    const currentRound = (0, import_last.default)(roundScore);
    if (!lastRoundScore) {
    ...
    const lastRound = (0, import_last.default)(lastRoundScore);
    if (!currentRound) {
  ...
  var getMyPlayerId = (pointInfo, username) => {
  ...
  var addHitsToCollisions = (collisions, hits) => {
    let hitIndex = 0;
  ...
  var roundParser = (round, username, isFirst) => {
    ...
    const allPoints = points.map((point) => {
      ...
      const pointObj = new point_default(hits, collisions, won, served);
      pointObj.collisions.forEach((c) => c.point = pointObj);
      return pointObj;
    }).filter((x) => x);
    const roundObj = new round_default(allPoints, false);
    allPoints.forEach((p) => p.round = roundObj);
    return roundObj;
  };
  var getOppenentAndIsFirst = (game, username) => {
  ...
  var gameParser = (game, username) => {
    ...
    const match = new match_default(opponent, rounds);
    rounds.forEach((r) => r.match = match);
    return match;
  };
  var getDateFromFilename = (filename) => {
  ...
  var fileParse = (contents, filename) => {
    ...
    const playSession = new play_session_default(getDateFromFilename(filename), games);
    games.forEach((g) => g.session = playSession);
    return playSession;
  };
  var playerSessions = (playSessions) => {
    return new play_sessions_default(playSessions.filter((x) => x));
  };

  // src/app.js
  var import_mean = __toESM(require_mean(), 1);
  var import_heatmap = __toESM(require_heatmap(), 1);
  var prettyPercentage = (float) => {
    return (float * 100).toFixed(2) + "%";
  };
  var prettyNumber = (float) => {
    return float.toFixed(2);
  };
  var statsRow = (session, date) => {
    const topspin = session.allHits.map((h) => h.topspin).filter((x) => x);
    const backspin = session.allHits.map((h) => h.backspin).filter((x) => x);
    const metersPerSecond = session.allHits.map((h) => h.metersPerSecond).filter((x) => x);
    ...
    clone.querySelectorAll("div")[1].innerHTML = prettyNumber((0, import_mean.default)(topspin));
    clone.querySelectorAll("div")[2].innerHTML = prettyNumber((0, import_mean.default)(backspin));
    clone.querySelectorAll("div")[3].innerHTML = prettyNumber(
      (0, import_mean.default)(metersPerSecond)
    );
    clone.querySelectorAll("div")[4].innerHTML = prettyNumber((0, import_mean.default)(revs));
    clone.querySelectorAll("div")[5].innerHTML = prettyPercentage(consistency);
    document.getElementById("table-rows").appendChild(clone);
  };
  var weekly = (sessions2) => {
    const byWeek = sessions2.byWeek();
    ...
    statsRow(sessions2, "All");
  };
  var sessions;
  window.sessions = sessions;
  var logsUpload = document.getElementById("logs-upload");
  logsUpload.onchange = function() {
    ...
    Promise.all(promises).then((values) => {
      ...
      heatmapInstance = import_heatmap.default.create({
  ...
  var flipOnAxis = (value, axisMax) => {
    return axisMax - value;
  };
  var heatmapInstance;
  var createHeatmap = (sessions2) => {
    const width = 400;
    const height = 350;
    const positions = {};
    ...
    sessions2.allMyHitsToTable(forehand, backhand, net, serve, rally).forEach((collision) => {
      let posx = parseInt((collision.posx + 0.8) * 250);
  ...
  var checkItem = document.querySelectorAll("[type='checkbox']");
